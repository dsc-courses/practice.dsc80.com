...
Each credential after the provider's name is preceded by `", "`, so splitting on `", "` gives one more piece than the number of credentials. Subtracting 1 gives the credential count.
...
Each credential beyond the first adds one `", "` to the string, so counting occurrences of `", "` gives the number of credentials.
